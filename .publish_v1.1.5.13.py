"""v1.1.5.13 发布脚本(raw binary POST)。"""
import json
import os
import re
import sys
...
REPO = "manju-x2"
TAG = "v1.1.5.13"
TITLE = "漫剧助手X-2 v1.1.5.13 — 装时 EXE 锁根因修复(restartreplace + PrepareToInstall)"
...
def _load_body():
    ...
    m = re.search(r"## v1\.1\.5\.13.*?(?=\n## v1\.1\.5\.12\b)", text, re.DOTALL)
    if not m:
        sys.exit("ERROR: docs/更新日志.md 找不到 v1.1.5.13 段")
    body = m.group(0).rstrip()
    setup = RELEASE_DIR / "X-2_v1.1.5.13_Setup.exe"
    md5 = (RELEASE_DIR / "X-2_v1.1.5.13_Setup.exe.md5").read_text(encoding="utf-8").strip()
    sha256 = (RELEASE_DIR / "X-2_v1.1.5.13_Setup.exe.sha256").read_text(encoding="utf-8").strip()
    size = setup.stat().st_size
    body += f"""

## 安装包校验

- 文件: `X-2_v1.1.5.13_Setup.exe`
...
> ⚠️ **重点**: 如果你之前装 v1.1.5.8 ~ v1.1.5.12 没生效(更新后还是老版本),这次新版修 EXE 锁问题,`PrepareToInstall` 装前会主动杀光所有锁 EXE 的进程,`restartreplace` 兜底改名替换,会真正完成 EXE 替换。
> ⚠️ 如果你**还停留在 v1.1.4 / v1.1.5.12**,需要先**手动**杀光所有 `漫剧助手X-2.exe` / `hermes.exe` / `python.exe`(任务管理器)再装 v1.1.5.13,虽然 `PrepareToInstall` 会自动杀,但手动杀一次更稳。
"""
    return body
...
def main():
    ...
    for name in ["X-2_v1.1.5.13_Setup.exe", "X-2_v1.1.5.13_Setup.exe.md5", "X-2_v1.1.5.13_Setup.exe.sha256"]:
