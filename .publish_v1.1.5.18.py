"""v1.1.5.18 发布脚本(raw binary POST)。"""
import json
import os
import re
import sys
...
REPO = "manju-x2"
TAG = "v1.1.5.18"
TITLE = "manju-x2 v1.1.5.18 - fix P0 hardcoded '1.1.4' UI version display"
...
def _load_body():
    ...
    m = re.search(r"## v1\.1\.5\.18.*?(?=\n## v1\.1\.5\.17\b)", text, re.DOTALL)
    if not m:
        sys.exit("ERROR: changelog not found")
    body = m.group(0).rstrip()
    setup = RELEASE_DIR / "X-2_v1.1.5.18_Setup.exe"
    md5 = (RELEASE_DIR / "X-2_v1.1.5.18_Setup.exe.md5").read_text(encoding="utf-8").strip()
    sha256 = (RELEASE_DIR / "X-2_v1.1.5.18_Setup.exe.sha256").read_text(encoding="utf-8").strip()
    size = setup.stat().st_size
    body += "\n\n## Installer\n\n"
    body += "- File: X-2_v1.1.5.18_Setup.exe\n"
...
def main():
    ...
    for name in ["X-2_v1.1.5.18_Setup.exe", "X-2_v1.1.5.18_Setup.exe.md5", "X-2_v1.1.5.18_Setup.exe.sha256"]:
