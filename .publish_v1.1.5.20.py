"""v1.1.5.20 发布脚本(raw binary POST)。"""
import json
import os
import re
import sys
...
REPO = "manju-x2"
TAG = "v1.1.5.20"
TITLE = "manju-x2 v1.1.5.20 - \u5206\u955c\u667a\u80fd\u4f53\u9009\u62e9(\u5206\u955c / \u5206\u955c2 \u5207\u6362)"
...
def _load_body():
    ...
    m = re.search(r"## v1\.1\.5\.20.*?(?=\n## v1\.1\.5\.19\b)", text, re.DOTALL)
    if not m:
        sys.exit("ERROR: changelog not found")
    body = m.group(0).rstrip()
    setup = RELEASE_DIR / "X-2_v1.1.5.20_Setup.exe"
    md5 = (RELEASE_DIR / "X-2_v1.1.5.20_Setup.exe.md5").read_text(encoding="utf-8").strip()
    sha256 = (RELEASE_DIR / "X-2_v1.1.5.20_Setup.exe.sha256").read_text(encoding="utf-8").strip()
    size = setup.stat().st_size
    body += "\n\n## Installer\n\n"
    body += "- File: X-2_v1.1.5.20_Setup.exe\n"
...
def main():
    ...
    for name in ["X-2_v1.1.5.20_Setup.exe", "X-2_v1.1.5.20_Setup.exe.md5", "X-2_v1.1.5.20_Setup.exe.sha256"]:
