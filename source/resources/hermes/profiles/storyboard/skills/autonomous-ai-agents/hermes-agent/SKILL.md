...
**Hermes Workspace deployment:** See `references/hermes-workspace-deployment.md` for deploying the Web UI frontend. For Swarm mode (multi-agent orchestration), see `references/hermes-workspace-swarm-setup.md` for concepts and `references/workspace-swarm-batch-setup.md` for the working batch automation that creates all 10 profiles in ~2 minutes (vs. 4-8 hours manual). If Workspace loads but shows "Authentication error" when you try to chat, see `references/hermes-workspace-api-auth.md` — you need to configure `API_SERVER_KEY` in `~/.hermes/.env` to enable Gateway's session API.
