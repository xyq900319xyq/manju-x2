...
REPO = "manju-x2"
TAG = "v1.1.5.3"
TITLE = "漫剧助手X-2 v1.1.5.3 — 升级后用户 API 配置不再丢失"
...
def _load_body():
    ...
    m = re.search(r"## v1\.1\.5\.3.*?(?=\n## v1\.1\.5\.2\b)", text, re.DOTALL)
    if not m:
        sys.exit("ERROR: docs/更新日志.md 找不到 v1.1.5.3 段")
    body = m.group(0).rstrip()
    setup = RELEASE_DIR / "X-2_v1.1.5.3_Setup.exe"
    md5 = (RELEASE_DIR / "X-2_v1.1.5.3_Setup.exe.md5").read_text(encoding="utf-8").strip()
    sha256 = (RELEASE_DIR / "X-2_v1.1.5.3_Setup.exe.sha256").read_text(encoding="utf-8").strip()
    size = setup.stat().st_size
    body += f"""

## 安装包校验

- 文件: `X-2_v1.1.5.3_Setup.exe`
...
覆盖安装即可,设置/数据/模型配置不丢。也可在软件内点"检查更新"一键升级。

## ⚠️ 注意

如果你已经装过 v1.1.5.2 时 API key 已经被覆盖丢失,**需要重新填一次 API key**(v1.1.5.3 修的是未来升级不再丢,过去丢的无法恢复)。
"""
    return body
...
def main():
    ...
    print()
    for fname in [
        "X-2_v1.1.5.3_Setup.exe",
        "X-2_v1.1.5.3_Setup.exe.md5",
        "X-2_v1.1.5.3_Setup.exe.sha256",
    ]:
