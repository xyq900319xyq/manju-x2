...
REPO = "manju-x2"
TAG = "v1.1.5"
TITLE = "漫剧助手X-2 v1.1.5 — 16 个 BUG 全面修复(1 致命+6 高+10 中)"
...
def _load_body():
    ...
    m = re.search(r"## v1\.1\.5.*?(?=\n## v1\.1\.4)", text, re.DOTALL)
    if not m:
        sys.exit("ERROR: docs/更新日志.md 找不到 v1.1.5 段")
    body = m.group(0).rstrip()
    setup = RELEASE_DIR / "X-2_v1.1.5_Setup.exe"
    md5 = (RELEASE_DIR / "X-2_v1.1.5_Setup.exe.md5").read_text(encoding="utf-8").strip()
    sha256 = (RELEASE_DIR / "X-2_v1.1.5_Setup.exe.sha256").read_text(encoding="utf-8").strip()
    size = setup.stat().st_size
    body += f"""

## 安装包校验

- 文件: `X-2_v1.1.5_Setup.exe`
...
def main():
    ...
    print()
    for fname in [
        "X-2_v1.1.5_Setup.exe",
        "X-2_v1.1.5_Setup.exe.md5",
        "X-2_v1.1.5_Setup.exe.sha256",
    ]:
