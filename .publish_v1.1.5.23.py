"""v1.1.5.23 发布脚本 — 修复资产列表漏抽"人物资产"段 BUG。

跟 .publish_v1.1.5.22.py 一样的 raw binary POST 流程,不 patch 旧 release。
"""
import json
import os
import re
...
REPO = "manju-x2"
NEW_TAG = "v1.1.5.23"
NEW_TITLE = "manju-x2 v1.1.5.23 - BUG 修复:资产列表漏抽人物资产段"
...
def _load_body():
    ...
    m = re.search(r"## v1\.1\.5\.23.*?(?=\n## v1\.1\.5\.22\b)", text, re.DOTALL)
    if not m:
        sys.exit("ERROR: changelog v1.1.5.23 not found")
    body = m.group(0).rstrip()
    setup = RELEASE_DIR / "X-2_v1.1.5.23_Setup.exe"
    if not setup.exists():
        sys.exit("ERROR: missing " + str(setup) + ",run build_x2.py first")
    md5 = (RELEASE_DIR / "X-2_v1.1.5.23_Setup.exe.md5").read_text(encoding="utf-8").strip()
    sha256 = (RELEASE_DIR / "X-2_v1.1.5.23_Setup.exe.sha256").read_text(encoding="utf-8").strip()
    size = setup.stat().st_size
    body += "\n\n## Installer\n\n"
    body += "- File: X-2_v1.1.5.23_Setup.exe\n"
...
def main():
    ...
    print("== publish v1.1.5.23 ==")

    # 1) 找 v1.1.5.23 release 是否已存在
    code, txt = _request("GET", api + "/releases/tags/" + NEW_TAG)
    if code == 200:
        rel = json.loads(txt)
        rel_id = rel["id"]
        for a in rel.get("assets", []):
            print("   - delete old v1.1.5.23 asset " + a["name"])
            _request("DELETE", api + "/releases/assets/" + str(a["id"]))
    elif code == 404:
        rel_id = None
    else:
        sys.exit("find v1.1.5.23 release failed: HTTP " + str(code))

    # 2) 创/更新 v1.1.5.23 release
    body = _load_body()
    ...
    if code not in (200, 201):
        sys.exit("release v1.1.5.23 failed: HTTP " + str(code) + " " + txt)
    rel = json.loads(txt)
    print("   OK release v1.1.5.23 " + ("created" if rel_id is None else "updated") + " url=" + rel.get("html_url", ""))

    # 3) 上传 v1.1.5.23 资产 (raw binary POST,不用 multipart - 硬约束 v1.1.1)
