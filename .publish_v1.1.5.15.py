"""v1.1.5.15 发布脚本(raw binary POST)。"""
import json
import os
import re
import sys
...
REPO = "manju-x2"
TAG = "v1.1.5.15"
TITLE = "manju-x2 v1.1.5.15 - Exec ewWaitUntilTerminated fix install"

ROOT = Path(r"D:\manju-helper\manju-x2")
...
def _load_body():
    ...
    m = re.search(r"## v1\.1\.5\.15.*?(?=\n## v1\.1\.5\.14\b)", text, re.DOTALL)
    if not m:
        sys.exit("ERROR: changelog not found")
    body = m.group(0).rstrip()
    setup = RELEASE_DIR / "X-2_v1.1.5.15_Setup.exe"
    md5 = (RELEASE_DIR / "X-2_v1.1.5.15_Setup.exe.md5").read_text(encoding="utf-8").strip()
    sha256 = (RELEASE_DIR / "X-2_v1.1.5.15_Setup.exe.sha256").read_text(encoding="utf-8").strip()
    size = setup.stat().st_size
    body += "\n\n## Installer\n\n"
    body += "- File: X-2_v1.1.5.15_Setup.exe\n"
...
def main():
    ...
    for name in ["X-2_v1.1.5.15_Setup.exe", "X-2_v1.1.5.15_Setup.exe.md5", "X-2_v1.1.5.15_Setup.exe.sha256"]:
